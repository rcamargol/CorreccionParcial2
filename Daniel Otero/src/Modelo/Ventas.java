...
public class Ventas {
	 ...
	 // metodo a construir
		...
		try {
			...
			while(linea != null) {
				...
				this.productos[i].setCodigoProducto(Integer.parseInt(ap[0]));
				this.productos[i].setNombreProducto(ap[1]);
				this.productos[i].setValorProducto(Integer.parseInt(ap[2]));
				linea = br.readLine();
				i++;
			}
	...
	public void consolidarVentas(double IVA) {
		...
		for(int i = 0;i < detalleVentas.length;i++) {
			cont += detalleVentas[i].getValorTotal();
			
		}
		this.valorTotal = cont + IVA;
	}
