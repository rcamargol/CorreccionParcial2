...
public class Ventas {
	...
	public void leerArchivoProductos(String nombrearchivo) {
		...
		nombrearchivo.setProperty("", "");
		nombrearchivo.setProperty("", "");
		try {

			nombrearchivo.store(new FileOutputStream(nombrearchivo), null);
	...
	public int generarReporteVentas(int codVentas) {
		int i;
		int valor1 = (Integer) null;
		for (i = 0; i < this.totalRegPR; i++) {
			if (detalleVentas[i].getCodigoDetalleVenta() == codVentas) {
				valor1 = detalleVentas[i].getCodigoDetalleVenta();
			}
		}
		return valor1;
	}
